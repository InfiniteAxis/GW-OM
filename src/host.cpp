...
int load_imm_host( )
{
	...
	if( !( fp = FileOpen( SYSTEM_DIR IMM_HOST_FILE, "r" ) ) )
	{
...
CMDF( do_add_imm_host )
{
	...
	if( type[0] == '\0' )
	{
		if( immortal_host_start == NULL )
		{
			...
			send_to_char( "Syntax: protect add    <name> <host>\r\n", ch );
			send_to_char( "Syntax: protect delete <name> <host>\r\n", ch );
			send_to_char( "Syntax: protect save\r\n", ch );
			return;
		}
		send_to_char( "Syntax: protect add    <name> <host>\r\n", ch );
		send_to_char( "Syntax: protect delete <name> <host>\r\n", ch );
...
void do_write_imm_host( void )
{
	...
	*/
	if( !( fp = FileOpen( SYSTEM_DIR IMM_HOST_FILE, "w" ) ) )
	{
		bug( "Do_write_imm_host: FileOpen" IMM_HOST_FILE, 0 );
