...
const char *const spell_flag[] = { "water", "art", "air", "astral", "area", "distant", "reverse",
...
bool check_skill( CHAR_DATA *ch, const char *command, const char *argument )
{
	...
	if( !skill_table[sn]->skill_fun )
	{
		...
		level = ( ( IS_NPC( ch ) || skill_table[sn]->guild < COMBAT_ABILITY
			|| skill_table[sn]->guild > ESPIONAGE_ABILITY ) ? ch->top_level : ch->skill_level[skill_table[sn]->guild] );
		...
		else if( skill_table[sn]->guild == ESPIONAGE_ABILITY )
			learn_from_limiter( ch, sn );
		else
...
CMDF( do_slookup )
{
	...
	else
	{
		...
		ch_printf( ch, "Combo: %s\r\n", skill->artcombo ? skill->artcombo : "" );

		if( skill->difficulty != '\0' )
			ch_printf( ch, "Difficulty: %d\r\n", ( int ) skill->difficulty );

		ch_printf( ch, "Type: %s  Target: %s  Minpos: %d  Beats: %d\r\n",
			skill_tname[skill->type],
			target_type[URANGE( TAR_IGNORE, skill->target, TAR_OBJ_INV )],
			skill->minimum_position, skill->beats );
...
CMDF( do_sset )
{
	...
	if( arg1[0] == '\0' || arg2[0] == '\0' || argument[0] == '\0' )
	{
		...
		if( get_trust( ch ) > LEVEL_LIAISON )
		{
			...
			send_to_char( "\n\rField being one of:\r\n", ch );
			send_to_char( "  name code target minpos slot beats dammsg wearoff guild minlevel\r\n", ch );
	...
	if( get_trust( ch ) > LEVEL_BUILDER && !str_cmp( arg1, "save" ) && !str_cmp( argument, "table" ) )
	{
			...
			|| ( is_number( arg1 ) && ( sn = atoi( arg1 ) ) >= 0 ) ) )
	{
		...
		if( !str_cmp( arg2, "lp" ) )
		{
			if( skill->guild != ESPIONAGE_ABILITY )
			{
				send_to_char( "That's not even an Espionage ability!\r\n", ch );
				return;
			}
			value = atoi( argument );
			if( value < 0 )
			{
				send_to_char( "Allowed range is 0 through 51.\r\n", ch );
				return;
			}
			skill->lp = value;
			send_to_char( "Ok.\r\n", ch );
		...
		if( !str_cmp( arg2, "affect" ) )
		{
			...
			while( argument[0] != 0 )
			{
				argument = one_argument( argument, bitv );
				if( ( tmpbit = get_aflag( bitv ) ) == -1 )
		...
		if( !str_cmp( arg2, "combo" ) )
		{
			if( skill->artcombo )
				DISPOSE( skill->artcombo );

			if( !str_cmp( argument, "clear" ) )
				skill->artcombo = str_dup( "" );
			else
				skill->artcombo = str_dup( argument );
			send_to_char( "Ok.\r\n", ch );
			return;
		}
...
{
	int adept, gain, sklvl, learn, percent, chance, guess;
	...
	if( ch->pcdata->learned[sn] < 100 )
	{
		...
		if( ( ch->pcdata->learned[sn] > 0 ) && ( ch->pcdata->learned[sn] <= 30 ) )
		{
			guess = number_range( 1, 3 );

			if( guess != 1 )
				return;
		}

		if( ( ch->pcdata->learned[sn] > 30 ) && ( ch->pcdata->learned[sn] <= 60 ) )
		{
			guess = number_range( 1, 2 );
		...
		if( ch->pcdata->learned[sn] == 100 )  /* fully learned! */
		{
			...
			ch_printf( ch, "You perfect %s!  You get %d experience for your success!\r\n", skill_table[sn]->name, gain );
		}
		else
		{
			gain = 100 * sklvl;
			...
			{
				set_char_color( AT_WHITE, ch );
				ch_printf( ch, "You earn %d extra experience!\r\n", gain );
			}
		}
...
{
	int adept, sklvl, learn, percent, schance, guess;

	if( IS_NPC( ch ) || ch->pcdata->learned[sn] == 0 )
		return;

	sklvl = skill_table[sn]->lp;

	if( skill_table[sn]->guild != ESPIONAGE_ABILITY )
		return;

	if( sklvl <= 45 )
		adept = ( ( ch->pcdata->lp - skill_table[sn]->lp ) * 10 ) + 50;
	else
		adept = ( ( ch->pcdata->lp - 45 ) * 10 ) + 50;
	adept = UMIN( adept, 100 );

	if( ch->pcdata->learned[sn] >= adept )
		return;

	if( sklvl == 0 || sklvl > ch->pcdata->lp )
		return;
	...
	if( ch->pcdata->learned[sn] < 100 )
	{
		...
		if( ch->pcdata->learned[sn] == 100 )  /* fully learned! */
		{
			...
			ch_printf( ch, "You perfect %s! Your bloodlust fuels your ability...\r\n", skill_table[sn]->name );
			ch->pcdata->lp += 1;
			if( ch->pcdata->lp >= 55 )
				ch->pcdata->lp = 55;
		}
...
CMDF( do_rush )
{
	...
	if( !IS_NPC( ch ) && !ch->pcdata->learned[gsn_rush] )
	{
	...
	if( IS_NPC( ch ) || percent < ch->pcdata->learned[gsn_rush] )
	{
		dam = number_range( 1000, ch->skill_level[COMBAT_ABILITY] * 3 );
		global_retcode = damage( ch, victim, dam, gsn_rush );
		...
		{
			act( AT_BLOOD, "Your rush crushes your victims torso!", ch, NULL, NULL, TO_CHAR );
		}
		if( global_retcode != rCHAR_DIED && global_retcode != rBOTH_DIED )
			learn_from_success( ch, gsn_rush );
	}
	else
	{
		WAIT_STATE( ch, skill_table[gsn_rush]->beats );
		global_retcode = damage( ch, victim, 0, gsn_rush );
		learn_from_failure( ch, gsn_rush );
	}
...
CMDF( do_dig )
{
	...
	separate_obj( obj );
	xREMOVE_BIT( obj->extra_flags, ITEM_BURRIED );
...
CMDF( do_steal )
{
	...
	{
		send_to_char( "You can't steal while holding weapons!\r\n", ch );
		return;
	}
	...
	if( !IS_NPC( ch ) && !xIS_SET( ch->act, PLR_PKER ) )
	{
	...
	{
		send_to_char( "Expect to be slayed for trying that.\r\n", ch );
		ch_printf( victim, "%s just tried to steal from you!\r\n", ch->name );
		return;
	}
	...
	if( !str_cmp( arg1, "dollars" ) || !str_cmp( arg1, "dollar" ) || !str_cmp( arg1, "money" ) )
	{
		int amount;
		...
		ch->gold += amount;
		victim->gold -= amount;
		ch_printf( ch, "Aha!  You got %d dollars.\r\n", amount );
	...
	if( !can_drop_obj( ch, obj )
		|| IS_OBJ_STAT( obj, ITEM_INVENTORY ) || IS_OBJ_STAT( obj, ITEM_ARTIFACT ) || IS_OBJ_STAT( obj, ITEM_PROTOTYPE ) )
	{
	...
	if( IS_NPC( victim ) );
	{
		xp =
			UMIN( obj->cost * 10,
				( exp_level( ch->skill_level[SMUGGLING_ABILITY] + 1 ) -
					exp_level( ch->skill_level[SMUGGLING_ABILITY] ) ) / 6 );
		xp = UMIN( xp, xp_compute( ch, victim ) );
		gain_exp( ch, xp, SMUGGLING_ABILITY );
		ch_printf( ch, "&WYou gain %ld smuggling experience!\r\n", xp );
	}
...
CMDF( do_rescue )
{
	...
	if( !can_see( ch, who_fighting( victim ) ) )
	{
		send_to_char( "You can't rescue them from something you can't see!.\r\n", ch );
		return;
	}
...
CMDF( do_bite )
{
	...
	if( IS_NPC( ch ) || number_percent( ) < ch->pcdata->learned[gsn_bite] )
	{
		...
		global_retcode = damage( ch, victim, number_range( 1, ch->skill_level[COMBAT_ABILITY] * 1.5 ), gsn_bite );
	}
	else
	{
...
CMDF( do_claw )
{
	...
	if( IS_NPC( ch ) || number_percent( ) < ch->pcdata->learned[gsn_claw] )
	{
		...
		global_retcode = damage( ch, victim, number_range( 1, ch->skill_level[COMBAT_ABILITY] * 1.5 ), gsn_claw );
	}
	else
	{
	...
	if( ( ship = ship_in_room( ch->in_room, arg ) ) != NULL )
	{
		...
		if( IS_NPC( ch ) || !ch->pcdata || number_percent( ) > ch->pcdata->learned[gsn_pickshiplock] )
		{
			...
			for( d = first_descriptor; d; d = d->next )
			{
				...
				ch_printf( victim, "&R[&PA&pL&PA&pR&PM&R] &P%s&p attempting to pick &P%s&p!\r\n", ch->name, ship->name );
			}
			return;
		}
		...
		if( !ship->hatchopen )
		{
			...
			for( d = first_descriptor; d; d = d->next )
			{
				...
				ch_printf( victim, "&R[&PA&pL&PA&pR&PM&R] &P%s&p just picked &P%s&p open!!\r\n", ch->name, ship->name );
			}
			return;
		}
	}
...
CMDF( do_sneak )
{
	...
	if( !IS_NPC( ch ) && ch->in_room && xIS_SET( ch->in_room->room_flags, ROOM_TAG ) )
	{
...
CMDF( do_recall )
{
	...
	*/
	...
	act( AT_ACTION, "$n appears in a swirl of mist.", ch, NULL, NULL, TO_ROOM );
	do_look( ch, "auto" );
...
bool check_parry( CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	if( !IS_NPC( victim ) && !IS_SET( victim->pcdata->flags, PCFLAG_GAG ) )
		/*SB*/ act( AT_SKILL, "You parry $n's attack.&w", ch, NULL, victim, TO_VICT );

	if( !IS_NPC( ch ) && !IS_SET( ch->pcdata->flags, PCFLAG_GAG ) )  /* SB */
		act( AT_SKILL, "$N parries your attack.&w", ch, NULL, victim, TO_CHAR );
...
bool check_dodge( CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	if( !IS_NPC( victim ) && !IS_SET( victim->pcdata->flags, PCFLAG_GAG ) )
		act( AT_SKILL, "You dodge $n's attack.&w", ch, NULL, victim, TO_VICT );

	if( !IS_NPC( ch ) && !IS_SET( ch->pcdata->flags, PCFLAG_GAG ) )
		act( AT_SKILL, "$N dodges your attack.&w", ch, NULL, victim, TO_CHAR );
...
CMDF( do_scan )
{
	...
	if( !argument || argument[0] == '\0' )
	{
		...
		send_to_char( "You scan your surroundings...\r\n", ch );
		act( AT_GREY, "$n scans $s surroundings.", ch, NULL, NULL, TO_ROOM );

		for( x = 0; x <= MAX_DIR; x++ )
		{
			if( ( pexit = get_exit( ch->in_room, x ) ) == NULL )
				continue;
			if( IS_SET( pexit->exit_info, EX_CLOSED ) )
				continue;
			if( room_is_private( ch, to_room ) && get_trust( ch ) < LEVEL_LIAISON )
				continue;
			if( IS_NPC( ch ) || ( number_percent( ) > ch->pcdata->learned[gsn_scan] ) )
			{
				act( AT_GREY, "You stop scanning $t as your vision blurs.", ch, dir_name[x], NULL, TO_CHAR );
				continue;
			}

			was_in_room = ch->in_room;
			act( AT_GREY, "Scanning $t...", ch, dir_name[x], NULL, TO_CHAR );
...
CMDF( do_blackjack )
{
	...
	if( victim == ch )
	{
		send_to_char( "How can you knock yourself unconscious?\r\n", ch );
		return;
	}
	...
	if( IS_IMMORTAL( ch ) || !str_cmp( ch->name, "Sedle" ) || percent < ch->pcdata->learned[gsn_blackjack] )
	{
		if( !IS_NPC( victim ) && ( ch->pcdata->learned[gsn_blackjack] < 60 ) )
		...
		act( AT_ACTION, "&c$n falls to the ground.", victim, NULL, NULL, TO_ROOM );
	}
	else
	{
...
CMDF( do_assassinate )
{
	...
	if( arg[0] == '\0' )
	{
		send_to_char( "\n\rAssassinate whom and what type?\r\n", ch );
		send_to_char( "Types: Poison, Choke, Backstab.\r\n", ch );
		return;
	}
	...
	{
		act( AT_LBLUE, "You blow posion powder in $N's face causing them to convulse as their bodies blood vessels explode.",
			ch, NULL, victim, TO_CHAR );
		act( AT_LBLUE, "$n blew poison powder in your face, causing you to convulse as your blood vessels explode.", ch, NULL,
			victim, TO_VICT );
		act( AT_LBLUE, "$n blows poison powder in $N's face causing convulsion as $N's blood vessels explode.", ch, NULL,
			victim, TO_NOTVICT );
	}
	else if( !str_cmp( arg2, "backstab" ) )
	{
		act( AT_RED, "Your backstab finds its mark between $N's shoulder blades.", ch, NULL, victim, TO_CHAR );
		act( AT_RED, "$n's backstab hits you square inbetween the shoulder blades, stealing your life.", ch, NULL, victim,
			TO_VICT );
		act( AT_RED, "$n backstabs $N in square inbetween the shoulder blades, stealing $N's life.", ch, NULL, victim,
			TO_NOTVICT );
	}
	else
	{    /*
	...
	percent = number_percent( ) - ( get_curr_lck( ch ) - 14 ) + ( get_curr_lck( victim ) - 3 );
	WAIT_STATE( ch, skill_table[gsn_assassinate]->beats );
	if( !IS_AWAKE( victim ) || percent < ch->pcdata->learned[gsn_assassinate] )
	{
...
CMDF( do_dropkick )
{
	...
	if( !IS_NPC( ch ) && ch->pcdata->learned[gsn_dropkick] <= 0 )
	{
		send_to_char( "Your mind races as you realize you have no idea how to do that.\r\n", ch );
		return;
	}
	...
	/*
	   chance = (((get_curr_dex(victim) + get_curr_str(victim))
		...
		if ( victim->fighting && victim->fighting->who != ch )
		  chance += 19;
	*/
	WAIT_STATE( ch, skill_table[gsn_dropkick]->beats );

	if( IS_NPC( ch ) || number_percent( ) < ch->pcdata->learned[gsn_dropkick] )
	{
		learn_from_success( ch, gsn_dropkick );
		/*
		 * do not change anything here!  -Thoric
		 */
		WAIT_STATE( ch, 3 * PULSE_VIOLENCE );
		...
		victim->position = POS_SITTING;
		global_retcode = damage( ch, victim, number_range( 1, ch->skill_level[COMBAT_ABILITY] * 1.5 ), gsn_dropkick );
	}
	else
	{
		WAIT_STATE( ch, 3 * PULSE_VIOLENCE );
		learn_from_failure( ch, gsn_dropkick );
		global_retcode = damage( ch, victim, 0, gsn_dropkick );
	}
	return;
}
...
CMDF( do_mace )
{
	...
	if( !IS_NPC( ch ) && ch->pcdata->lp < 44 )
	{
		send_to_char( "&RYou need atleast 44 LP to use this skill!\r\n", ch );
		return;
	}
	...
	if( !IS_NPC( ch ) && !ch->pcdata->learned[gsn_mace] )
	{
		send_to_char( "You do not yet know of this skill.\r\n", ch );
		return;
	}
	...
	if( IS_NPC( ch ) || percent < ch->pcdata->learned[gsn_mace] )
	{
		if( !IS_AFFECTED( victim, AFF_BLIND ) )
		{
			af.type = gsn_blindness;
			af.location = APPLY_HITROLL;
			af.modifier = -6;
			af.duration = 3 + ( ch->top_level / 3 );
			af.bitvector = AFF_BLIND;
			affect_to_char( victim, &af );
			ch_printf( victim, "&R%s sprays mace in your eyes! It STINGS!!!\r\n", ch->name );
			ch_printf( ch, "&RYou blast %s's eyes with Mace!\r\n", victim->name );
			act( AT_PLAIN, "&R$n sprays mace into $N's eyes!", ch, NULL, victim, TO_ROOM );
		}
		WAIT_STATE( ch, PULSE_VIOLENCE );
		learn_from_limiter( ch, gsn_mace );
	}
	else
	{
		ch_printf( victim, "&R%s just tried to spray mace in your eyes!!!\r\n", ch->name );
		ch_printf( ch, "&RYou try to spray %s's eyes with your Mace!\r\n", victim->name );
		act( AT_PLAIN, "&R$n tries to spray $N's eyes with mace but misses!", ch, NULL, victim, TO_ROOM );
		WAIT_STATE( ch, skill_table[gsn_mace]->beats );
	}
	return;
}
...
CMDF( do_pdust )
{
	...
	if( !IS_NPC( ch ) && ch->pcdata->lp < 40 )
	{
		send_to_char( "&RYou need atleast 40 LP to use this skill!\r\n", ch );
		return;
	}
	...
	if( !IS_NPC( ch ) && !ch->pcdata->learned[gsn_pdust] )
	{
		send_to_char( "You do not yet know of this skill.\r\n", ch );
		return;
	}
	...
	if( IS_NPC( ch ) || percent < ch->pcdata->learned[gsn_pdust] )
	{
		if( !IS_AFFECTED( victim, AFF_POISON ) )
		{
			...
			act( AT_PLAIN, "&R$n opens his palm and blows poison dust in your face!\r\n&GYou feel strange!", ch, NULL, victim,
				TO_VICT );
			act( AT_PLAIN, "&R$n blows poison dust into $N's face!", ch, NULL, victim, TO_NOTVICT );
			act( AT_PLAIN, "&RYou open your palm and blow poison dust into $N's face!", ch, NULL, victim, TO_CHAR );

		}
		WAIT_STATE( ch, PULSE_VIOLENCE );
		learn_from_limiter( ch, gsn_pdust );
	}
	else
	{
		act( AT_PLAIN, "&YYou luckily dodge $n's poison dust!", ch, NULL, victim, TO_VICT );
		act( AT_PLAIN, "&R$n's poison dust misses $N's face!", ch, NULL, victim, TO_NOTVICT );
		act( AT_PLAIN, "&RYour poison dust misses $N's face!", ch, NULL, victim, TO_CHAR );
...
CMDF( do_rub )
{
	...
	{
		send_to_char( "You can already see!\r\n", ch );
		return;
	}

	WAIT_STATE( ch, skill_table[gsn_rub]->beats );
	if( IS_NPC( ch ) || number_percent( ) < ch->pcdata->learned[gsn_rub] )
	{
		affect_strip( ch, gsn_blindness );
		act( AT_SKILL, "You rub your eyes clean.", ch, NULL, NULL, TO_CHAR );
		act( AT_SKILL, "$n rubs $s eyes clean.", ch, NULL, NULL, TO_NOTVICT );
		learn_from_success( ch, gsn_rub );
		return;
	}
	else
	{
		act( AT_SKILL, "You try to rub your eyes clean, but it doesn't work.", ch, NULL, NULL, TO_CHAR );
		act( AT_SKILL, "$n tries to rub $s eyes clean, but fails.", ch, NULL, NULL, TO_NOTVICT );
		learn_from_failure( ch, gsn_rub );
		return;
	}
	return;
}
...
CMDF( do_strangle )
{
	...
	if( ch->mount )
	{
		send_to_char( "You can't strangle while mounted.\r\n", ch );
		return;
	}
	...
	if( victim == ch )
	{
		send_to_char( "You want to strangle yourself?\r\n", ch );
		return;
	}
	...
	if( ( obj = get_eq_char( ch, WEAR_WIELD ) ) == NULL
		|| ( obj->value[3] != 4 ) )
	{
		send_to_char( "You need to be wielding a whip.\r\n", ch );
		return;
	}

	if( !ch->fighting )
	{
		send_to_char( "You can't strangle when you aren't fighting.\r\n", ch );
		return;
	}

	if( !victim->fighting )
	{
		send_to_char( "Get into a fight first.\r\n", ch );
		return;
	}
	...
	WAIT_STATE( ch, skill_table[gsn_circle]->beats );
	if( percent < ( IS_NPC( ch ) ? ( ch->skill_level[HUNTING_ABILITY] * 1.5 )
		: ch->pcdata->learned[gsn_strangle] ) )
	{
		learn_from_success( ch, gsn_strangle );
		global_retcode = multi_hit( ch, victim, gsn_strangle );

		chance = IS_NPC( ch ) ? ch->top_level
			: ( int ) ( ( ch->pcdata->learned[gsn_second_attack] ) / 1.5 );
		...
		chance = IS_NPC( ch ) ? ch->top_level
			: ( int ) ( ( ch->pcdata->learned[gsn_third_attack] ) / 1.5 );
		if( number_percent( ) < chance )
		{
			learn_from_success( ch, gsn_second_attack );
			global_retcode = multi_hit( ch, victim, gsn_strangle );
	...
	{
		learn_from_failure( ch, gsn_strangle );
		global_retcode = damage( ch, victim, 0, gsn_strangle );
	}
	return;
}

/*
 * New skills command that allows players to see which skills they have learned in a
 * specific ability - Diablo 2013
 */ CMDF( do_skills ) {
	 int sn;
	 char skn[MAX_INPUT_LENGTH];
	 char arg[MAX_INPUT_LENGTH];
	 int sAbil = 0;
	 int col = 0, cnt = 0, scnt = 0;
	 int ability;
	 if( IS_NPC( ch ) )
		 return;
	 argument = one_argument( argument, arg );
	 if( arg[0] == '\0' )
	 {
		 send_to_char( "&PSyntax&p: &pskills &P<&zability name&P>&D\r\n", ch );
		 return;
	 }
	 //   if( !str_cmp( arg, "all" ) )
	 //      sAbil = -1;
	 ...
	 else
	 {
		 ...
		 {
			 if( !str_cmp( ability_name[ability], arg ) )
			 {
				 sAbil = ability;
				 break;
			 }
		 ...
		 {
			 send_to_char( "&RThat's not a valid ability name! Try typing LEVEL.\r\n", ch );
			 return;
		 }
	 }
	 send_to_pager( "&pCurrently Learned Skills&P:&D \r\n", ch );
	 send_to_pager( "&P-----------------------------------------------------------------------&D\r\n", ch );
	 for( ability = -1; ability < MAX_ABILITY; ability++ )
	 {
		 if( sAbil >= 0 && ability != sAbil )
			 continue;
		 ...
		 for( sn = 0; sn < top_sn; sn++ )
		 {
			 ...
			 cnt++;
			 scnt++;
			 pager_printf( ch, " &p%18.18s &P(&z%3d%%&P)&D ", skill_table[sn]->name, ch->pcdata->learned[sn] );
			 if( ++col % 3 == 0 )
				 send_to_char( "\r\n", ch );
		 }
		 if( col != 0 )
		 ...
		 if( scnt <= 0 )
			 pager_printf( ch, " &RNo currently known skills.&D\r\n" );
	 }
	 send_to_pager( "&P-----------------------------------------------------------------------&D\r\n", ch );
	 pager_printf( ch, "&pYou currently know &P%d&p skills.&D\r\n", cnt );
	 return;
 }
