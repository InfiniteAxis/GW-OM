...
int mprog_do_ifcheck( const char *ifcheck, CHAR_DATA *mob, CHAR_DATA *actor, OBJ_DATA *obj, void *vo, CHAR_DATA *rndm )
{
	...
	*pchck = '\0';
	...
	if( chkchar )
	{
		...
		{
			return ( get_trust( chkchar ) >= LEVEL_STAFF );
		}
		...
		{
			return xIS_SET( chkchar->in_room->room_flags, ROOM_NO_RECALL ) ? true :
				false;
		}
		if( !str_cmp( chck, "sex" ) )
		{
		...
		if( !str_cmp( chck, "wearing" ) )
		{
			...
			for( tobj = chkchar->first_carrying; tobj; tobj = tobj->next_content )
			{
				...
				*/
				if( chkchar == tobj->carried_by
					&& tobj->wear_loc > -1
					&& !str_cmp( rval, w_flags[tobj->wear_loc] ) )
					return true;
			}
			return false;
		}
...
void mprog_driver( const char *com_list, CHAR_DATA *mob, CHAR_DATA *actor, OBJ_DATA *obj, void *vo, bool single_step )
{
	char tmpcmndlst[MAX_STRING_LENGTH], arg[MSL];
	char *command_list;
	char *cmnd;
	CHAR_DATA *rndm = NULL;
	CHAR_DATA *vch = NULL;
...
void oprog_put_trigger( CHAR_DATA *ch, OBJ_DATA *obj, OBJ_DATA *tobj )
{
	...
	{
		set_supermob( obj );
		oprog_percent_check( supermob, ch, obj, tobj, PUT_PROG );
		release_supermob( );
	}
	return;
}
...
void progbug( const char *str, CHAR_DATA *mob )
{
	...
	{
		snprintf( buf, MSL, "%s, Mob #%d.", str, mob->pIndexData->vnum );
	}
