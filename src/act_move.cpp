...
const char *const room_sents[SECT_MAX][25] = {
...

   {
	...
	"Some street people are putting on an interesting display of talent trying to earn some money.",
...
ch_ret move_char( CHAR_DATA *ch, EXIT_DATA *pexit, int fall )
{
	...
	if( !IS_NPC( ch ) && room_is_private( ch, to_room ) && !IS_SUPREME( ch ) )
	{
	...
	if( ch->plr_home == to_room )
	{
		if( get_timer( ch, TIMER_RECENTFIGHT ) > 0 && !IS_IMMORTAL( ch ) )
		{
			if( xIS_SET( ch->act, PLR_PKER ) )
			{
				...
				send_to_char( "Sorry! Try getting into your house after you've cooled down.\r\n", ch );
				return rNONE;
			}
		}
	}
	...
	if( !IS_NPC( ch ) && IS_SET( ch->pcdata->flags, PCFLAG_AUTOSCAN ) )
	{
		send_to_char( "\r\n", ch );
		do_scan( ch, dir_name[door] );
	}
...
void trapdamage( CHAR_DATA *ch, OBJ_DATA *obj )
{
	...
	{
		if( obj->short_descr && nifty_is_name( wch->name, obj->short_descr ) )
		{
			send_to_char( "&YSomeone set off your trap!\r\n", wch );
			break;
		}
	}
	...
	case 2:
		ch->move -= 150;
		send_to_char
		( "&YYou feel a sharp pain shoot through your leg, just before realising you stepped into a bear style trap!\r\n",
			ch );
		act( AT_BLUE, "Everyone laugh at $n! The sucker just stepped into a bear style trap! Ha!", ch, NULL, NULL,
			TO_ROOM );
		dam = number_range( obj->value[1], obj->value[1] * 10 );
	...
	case 3:
		if( !IS_AFFECTED( ch, AFF_BLIND ) )
		{
			...
			act( AT_SKILL, "&RYou spring a trip wire, acid comes pouring out onto your eyes blinding you!", ch, NULL, NULL,
				TO_CHAR );
		}
		act( AT_BLUE, "$n hits a trip wire, acid pours from the ceiling into $s eyes!", ch, NULL, NULL, TO_ROOM );
		break;
	...
	case 4:
		if( !IS_AFFECTED( ch, AFF_PARALYSIS ) )
		{
			...
			affect_to_char( ch, &af );
			act( AT_SKILL, "&RYou spring a trip wire, and an energy net comes down encasing you in it!", ch, NULL, NULL,
				TO_CHAR );
			update_pos( ch );
		}
		act( AT_BLUE, "$n hits a trip wire, an energy net comes down, encasing $m in it!", ch, NULL, NULL, TO_ROOM );
		break;
	...
	case 5:
		if( !IS_AFFECTED( ch, AFF_POISON ) )
		{
			...
			affect_to_char( ch, &af );
			act( AT_SKILL, "&RYou spring a trip wire, and a poison tipped dart flies out, hitting you in the side!", ch,
				NULL, NULL, TO_CHAR );
		}
		act( AT_BLUE, "$n hits a trip wire, as a poison tipped dart flies out into $s side!", ch, NULL, NULL, TO_ROOM );
		break;

	case 6:
		send_to_char( "&YYou trip a small wire! You are smashed in the face by a backlash trap!!\r\n", ch );
		act( AT_BLUE, "Everyone laugh at $n! $e just got smashed in the face by a backlash trap!", ch, NULL, NULL,
			TO_ROOM );
		dam = number_range( obj->value[1] * 2, obj->value[1] * 15 );
...
CMDF( do_bashdoor )
{
	...
	if( ( ch->in_room->vnum == ROOM_VNUM_HELL ) && ( !IS_IMMORTAL( ch ) ) )
	{
...
CMDF( do_sit )
{
	...
	if( obj != NULL )
	{
		if( obj->item_type != ITEM_FURNITURE || ( !obj->value[2] ) )
		{
...
CMDF( do_sleep )
{
	...
	case POS_STANDING:
		...
		else   /* find an object and sleep on it */
		{
			...
			{
				act( AT_ACTION, "There is no room on $p for you.", ch, obj, NULL, TO_CHAR );
				return;
			}
...
void do_sprint( CHAR_DATA *ch, const char *argument )
{
	...
	if( ch->position == POS_FIGHTING )
	{
		send_to_char( "&RYou are a little busy right now.\r\n", ch );
		return;
	}

	if( ch->mount )
	{
		send_to_char( "&RYou cannot sprint while mounted!\r\n", ch );
		return;
	}

	if( arg[0] == '\0' )
	{
		send_to_char( "Sprint where?\r\n", ch );
		return;
	}

	if( ch->position != POS_STANDING )
	{
		send_to_char( "&RYou must be standing up before you sprint!\r\n", ch );
		return;
	}

	if( !IS_NPC( ch ) )
		schance = ch->pcdata->learned[gsn_sprint] / 2;
	else
		schance = 90;

	if( number_percent( ) > schance )
	{
		send_to_char( "&RYou fail to coordinate yourself to do that...\r\n", ch );
		ch->move -= URANGE( 3, ch->move, 6 );
		return;
	}
	...
	if( number_percent( ) < schance )
	{
		...
		while( ( pexit = find_door( ch, arg, true ) ) != NULL )
		{
			if( ch->move < 1 )
			{
				send_to_char( "&RYou are too exhausted to sprint...\r\n", ch );
				ch->move = 0;
				return;
			}

			ch->move -= URANGE( 1, ch->move, 3 );

			learn_from_success( ch, gsn_sprint );
		...
		{
			send_to_char( "&RYou try to sprint but don't get anywhere.\r\n", ch );
			act( AT_ACTION, "$n tries to sprint but doesn't get anywhere.", ch, NULL, NULL, TO_ROOM );
			return;
		}

		send_to_char( "&YYou slow down after you sprint.\r\n", ch );
		act( AT_ACTION, "$n slows down after $s sprint.", ch, NULL, NULL, TO_ROOM );
		return;
	}

	send_to_char( "&RYou fail to coorinate yourself to sprint...\r\n", ch );
	ch->move -= URANGE( 3, ch->move, 6 );
	return;
}
