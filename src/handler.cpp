...
void explode( OBJ_DATA *obj )
{
	if( obj->armed_by )
	{
		...
		{
			if( !IS_NPC( vch ) && nifty_is_name( obj->armed_by, vch->name ) )
				break;
		}
	...
	else
	{
		if( obj->in_room )
		{
			if( obj->in_room->first_person )
				act( AT_WHITE, "The fuse on $p fizzles out and dies, falling apart. Phew! A dud.",
					obj->in_room->first_person, obj, NULL, TO_ROOM );
		}
		else if( obj->carried_by )
		{
			act( AT_WHITE, "$p falls apart in your hands... Must've been a dud.", obj->carried_by, obj, NULL, TO_CHAR );
			act( AT_WHITE, "$p falls apart in $n's hands. Phew! A dud.", obj->carried_by, obj, NULL, TO_ROOM );
		}
	}
	make_scraps( obj );
	return;
...
void room_explode( OBJ_DATA *obj, CHAR_DATA *vch, ROOM_INDEX_DATA *room )
{
	int blast;

	blast = ( int ) ( obj->value[1] / 500 );
	room_explode_1( obj, vch, room, blast );
	room_explode_2( room, blast );
}

void room_explode_1( OBJ_DATA *obj, CHAR_DATA *vch, ROOM_INDEX_DATA *room, int blast )
{
	CHAR_DATA *rch;
	CHAR_DATA *rnext;
	OBJ_DATA *robj;
	...
	for( rch = room->first_person; rch; rch = rnext )
	{
		...
		if( IS_NPC( rch ) || ( !IS_NPC( rch ) && xIS_SET( rch->act, PLR_PKER ) ) )
		{
			...
			if( !char_died( rch ) )
			{
				if( IS_NPC( rch ) && vch )
				{
					if( xIS_SET( rch->act, ACT_SENTINEL ) )
					{
						rch->was_sentinel = rch->in_room;
						xREMOVE_BIT( rch->act, ACT_SENTINEL );
					}
					start_hating( rch, vch );
					start_hunting( rch, vch );
				}
			}
		}
	}
		...
		for( pexit = room->first_exit; pexit; pexit = pexit->next )
		{
			if( pexit->to_room && pexit->to_room != room )
			{
				if( blast > 0 )
				{
					int roomblast;
					roomblast = blast - 1;
					room_explode_1( obj, vch, pexit->to_room, roomblast );
				}
				else
...
short get_trust( CHAR_DATA *ch )
{
	...
	if( ch->top_level >= LEVEL_STAFF && IS_RETIRED( ch ) )
		return LEVEL_STAFF;
...
{
	return 5 + ( ch->played + ( current_time - ch->logon ) ) / 14400;
}
...
int can_carry_n( CHAR_DATA *ch )
{
	...
	if( !IS_NPC( ch ) && get_trust( ch ) >= LEVEL_STAFF )
...
void affect_modify( CHAR_DATA *ch, AFFECT_DATA *paf, bool fAdd )
{
	...
	case APPLY_REMOVESPELL:
		if( xIS_SET( ch->in_room->room_flags, ROOM_NO_MAGIC ) || IS_SET( ch->immune, RIS_MAGIC )
			...
			|| saving_char == ch || loading_char == ch )    /* so saving/loading doesn't trigger */
			return;

		mod = abs( mod );
		if( IS_VALID_SN( mod ) && ( skill = skill_table[mod] ) != NULL && skill->type == SKILL_SPELL )
			if( ( retcode = ( *skill->spell_fun ) ( mod, ch->skill_level[COMBAT_ABILITY], ch, ch ) ) == rCHAR_DIED
				|| char_died( ch ) )
				return;
		break;
...
void char_to_room( CHAR_DATA *ch, ROOM_INDEX_DATA *pRoomIndex )
{
	...
	if( !IS_NPC( ch ) && xIS_SET( ch->in_room->room_flags, ROOM_SAFE ) && get_timer( ch, TIMER_SHOVEDRAG ) <= 0 )
...
void extract_char( CHAR_DATA *ch, bool fPull )
{
	...
	if( !ch->in_room )
	{
		bug( "Extract_char: %s in NULL room. Transferring to Limbo.", ch->name ? ch->name : "???" );
	...
	if( ch->desc && ch->desc->original && xIS_SET( ch->act, ACT_POLYMORPHED ) )
		do_revert( ch, "" );
...
bool room_is_private( CHAR_DATA *ch, ROOM_INDEX_DATA *pRoomIndex )
{
	...
	{
		if( ch->plr_home != pRoomIndex )
			if( pRoomIndex->guests && !nifty_is_name( ch->name, pRoomIndex->guests ) )
				return true;
	}
...
bool can_see( CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	*/
	...
	//    if( !IS_SUPREME(ch) && victim == supermob )
	//  return false;

	if( ch == victim )
		return true;

	//   if ( ( xIS_SET(ch->in_room->room_flags2, ROOM_TAG ) )
	//      && xIS_SET(victim->in_room->room_flags2, ROOM_TAG) )
	//      return true;
...
void fix_char( CHAR_DATA *ch )
{
	...
	ch->mental_state = 0;
	ch->max_hit = ( ( ( ( ch->skill_level[COMBAT_ABILITY] +
		ch->skill_level[PILOTING_ABILITY] + ch->skill_level[ENGINEERING_ABILITY] +
		ch->skill_level[HUNTING_ABILITY] + ch->skill_level[SMUGGLING_ABILITY] +
		ch->skill_level[DIPLOMACY_ABILITY] + ch->skill_level[LEADERSHIP_ABILITY] +
		ch->skill_level[ESPIONAGE_ABILITY] ) * number_range( 7, 12 ) ) + 500 ) );
...
int strlen_color( const char *argument )
{
	...
	bool wasup = false;
	bool valid = false;
	const char *validamp = "RrBbGgYOWwPpCczx/tT?";
	const char *validup = "rbgOzwpcx";
...
void check_lottery( CHAR_DATA *ch )
{
	...
	if( IS_SET( ch->pcdata->flags, PCFLAG_LOTTOWAIT ) && ( ch->pcdata->ticketweek < sysdata.lotteryweek ) )
	{
		send_to_char( "You can purchase another lottery ticket now.\r\n", ch );
		REMOVE_BIT( ch->pcdata->flags, PCFLAG_LOTTOWAIT );
		ch->pcdata->ticketweek = ( sysdata.lotteryweek + 1 );
		return;
	}
	...
	if( IS_SET( ch->pcdata->flags, PCFLAG_HASLOTTO ) )
	{
		if( ch->pcdata->ticketweek < sysdata.lotteryweek )
		{
			send_to_char( "Your lottery ticket has expired. Please purchase another.\r\n", ch );
	...
	{
		send_to_char( "You have a losing number. Please try again.\r\n", ch );
		REMOVE_BIT( ch->pcdata->flags, PCFLAG_HASLOTTO );
		//     ch->pcdata->ticketweek = (sysdata.lotteryweek + 1 );
		SET_BIT( ch->pcdata->flags, PCFLAG_LOTTOWAIT );
		return;
	}
...
bool exists_player( const char *name )
{
	...
	if( stat( buf, &fst ) != -1 )
		return true;
	else if( ( victim = get_char_world( supermob, name ) ) != NULL && !IS_NPC( victim ) )
		return true;
...
/*bool has_rate( CHAR_DATA *ch, int rate )
{
	...
	if( rate < 0 && ch->top_level < 100 )
		return true;

	if( ch->pcdata->rprate >= rate )
		return true;
