...
typedef struct renumberArea RENUMBER_AREA;
...
CMDF( do_renumber )
{
	 ...
	 * sanity check
	 */
	if( new_base == area->low_vnum && new_base == area->low_vnum && new_base == area->low_vnum && !fill_gaps )
	{
		ch_printf( ch,
	...
	if( ch->top_level == LEVEL_LIAISON )
	{
		if( area->low_vnum < ch->pcdata->low_vnum || area->hi_vnum > ch->pcdata->hi_vnum ||
			area->low_vnum < ch->pcdata->low_vnum || area->hi_vnum > ch->pcdata->hi_vnum ||
			area->low_vnum < ch->pcdata->low_vnum || area->hi_vnum > ch->pcdata->hi_vnum )
		{
	...
	{
		if( r_area->low_room < area->low_vnum || r_area->hi_room > area->hi_vnum ||
			r_area->low_obj < area->low_vnum || r_area->hi_obj > area->hi_vnum ||
			r_area->low_mob < area->low_vnum || r_area->hi_mob > area->hi_vnum )
		{
			ch_printf( ch,
...
bool check_vnums( CHAR_DATA *ch, AREA_DATA *tarea, RENUMBER_AREA *r_area )
{
	...
	while( area )
	{
		if( tarea == area )
			;
		else if( !( high < area->low_vnum || low > area->hi_vnum ) ||
			!( high < area->low_vnum || low > area->hi_vnum ) ||
			!( high < area->low_vnum || low > area->hi_vnum ) )
		{
