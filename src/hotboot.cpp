...
void save_world( CHAR_DATA *ch )
{
	...
	if( ( mobfp = FileOpen( filename, "w" ) ) == NULL )
	{
		bug( "%s: FileOpen mob file", __func__ );
		perror( filename );
	}
	else
		mobfile++;
	...
	if( ( shipfp = FileOpen( filename, "w" ) ) == NULL )
	{
		bug( "%s: FileOpen ship file", __func__ );
		perror( filename );
	}
	else
		shipfile++;
	...
	for( iHash = 0; iHash < MAX_KEY_HASH; iHash++ )
	{
		for( pRoomIndex = room_index_hash[iHash]; pRoomIndex; pRoomIndex = pRoomIndex->next )
		{
			if( pRoomIndex )
			{
				...
				{
					bug( "%s: FileOpen %d", __func__, pRoomIndex->vnum );
					perror( filename );
					continue;
				}
...
void load_world( CHAR_DATA *ch )
{
	...
	if( ( mobfp = FileOpen( file1, "r" ) ) == NULL )
	{
		bug( "%s: FileOpen mob file", __func__ );
		perror( file1 );
	}
	...
	if( ( shipfp = FileOpen( file2, "r" ) ) == NULL )
	{
		bug( "%s: FileOpen ship file", __func__ );
		perror( file1 );
	}
...
CMDF( do_hotboot )
{
	...
	mudstrlcpy( buf, "\r\nA Blinding flash of light covers the universe!", 100 );
	/*
	...
	execl( EXE_FILE, "gwom", buf, "hotboot", buf2, buf3, ( char * ) NULL );
...
void hotboot_recover( void )
{
	...
	for( ;; )
	{
		 ...
		 */
		if( !write_to_descriptor_old( desc, "\r\nThe light fades and you're back to normal!\r\n", 0 ) )
		{
			close( desc ); /* nope */
			continue;
		}
		...
		{
			write_to_descriptor( d, "\r\nSomehow, your character was lost in the hotboot. Please contact an Immortal.\r\n", 0 );
			close_socket( d, false );
		}
		else  /* ok! */
		{
			write_to_descriptor( d, "\n\rWhat was that light..\r\n", 0 );
