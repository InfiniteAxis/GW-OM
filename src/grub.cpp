...
const char *owear_to_disp( short arg )
{
	static const char owear_disp[20][3] =
...
int go_fnam_to_num( const char *arg )
{
	int cou;
	...
	CHAR_DATA *ch, int op_num, bool or_sw, int dis_num )
{
	...
	char buf_sex[] = "NMF";                     /* convert sex to text       */
	char buf_class[] = "MCTWVDRAPN";              /* convert class to text     */
	char buf_race[][3] =                          /* convert race to text      */
...
#ifdef WIN32
	if( ( fp = FileOpen( "\\smaug\\grub.dat", "rb" ) ) == NULL )
#else
	if( ( fp = FileOpen( "~/swr/grub.new", "r" ) ) == NULL )
#endif
		return;
		...
		if( res )                             /* record is a match         */
		{
			...
			if( tot_match <= dis_num )         /* print record if applicable */
				ch_printf( ch,
					...
					r.name, r.level, buf_sex[( unsigned char ) r.sex],
					buf_race[( unsigned char ) r.race], buf_class[( unsigned char ) r.pClass],
					buf_clan[( unsigned char ) r.clan],
					buf_council[( unsigned char ) r.council],
...
//				px = po->pIndexData;
	...
	{
		MOB_INDEX_DATA *pm;
		short srace, dis_num, vnum1, vnum2, dis_cou = 0;
