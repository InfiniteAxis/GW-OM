...
void show_char_to_char_0(CHAR_DATA *victim, CHAR_DATA *ch)
{
	...
	if (!IS_NPC(victim) && IS_SET(victim->pcdata->tag_flags, TAG_FROZEN) && IS_SET(victim->pcdata->tag_flags, TAG_PLAYING))
...
CMDF(do_look)
{
	...
	if ((pexit = find_door(ch, arg1, true)) != NULL)
	{
		 ...
		 */
		if (pexit->to_room && (IS_AFFECTED(ch, AFF_SCRYING) || IS_SET(pexit->exit_info, EX_xLOOK) || get_trust(ch) >= LEVEL_STAFF))
		{
...
CMDF(do_time)
{
	...
	if (IS_IMMORTAL(ch))
	{
		ch_printf(ch, "&YThe mud started up at:&O    %s\r"
					  "&YNext Reboot is set for:&O   %s\r",
				  str_boot_time, reboot_time);
		return;
	}
	return;
}
...
CMDF(do_who)
{
	...
	for (wlevel = 1; wlevel <= 2001; wlevel++)
	{
		...
		for (d = last_descriptor; d; d = d->prev)
		{
			...
			if (wch->top_level < iLevelLower || wch->top_level > iLevelUpper || (fImmortalOnly && wch->top_level < LEVEL_STAFF) || (fRaceRestrict && !rgfRace[wch->race]) || (fClanMatch && (pClan != wch->pcdata->clan)) /* SB */)
				continue;
			...
			if (IS_IMMORTAL(wch))
				snprintf(lev, 4, "%d", wch->top_level);
			else
			...
			snprintf(whobuf, MSL * 5, "&C[&c%4s %s&C]&W %s%s%s%s %s%s%s %s%s%s %s\r\n",
					 ...
					 char_name,
					 IS_NPC(wch) ? "" : xIS_SET(wch->act, PLR_AFK) ? wch->pcdata->afkmess
																   : wch->pcdata->title,
					 extra_title, clan_name, xIS_SET(wch->act, PLR_KILLER) ? "&P[&RW&rF&RM&P] &W" : "&W", dnd);
			/*
...
CMDF(do_where)
{
	...
	if (arg[0] == '\0')
	{
		...
		for (d = first_descriptor; d; d = d->next)
			if ((d->connected == CON_PLAYING || d->connected == CON_EDITING) && (victim = d->character) != NULL && !IS_NPC(victim) && victim->in_room && (victim->in_room->area == ch->in_room->area || get_trust(ch) >= LEVEL_STAFF) && can_see(ch, victim))
			{
				found = true;
	...
	if (argument[0] == '\0')
	{
		for (hash = 0; hash < 126; hash++)
			for (command = command_hash[hash]; command; command = command->next)
				if (command->level < LEVEL_HERO && command->level <= get_trust(ch) && (command->name[0] != 'm' || command->name[1] != 'p') && command->cshow == 0)
				{
					nCode = false;
					nHelp = false;
...
CMDF(do_afk)
{
	...
	if (!str_cmp(argument, "boo"))
	{
		...
		snprintf(buf, MAX_STRING_LENGTH, "&W[&zA&WF&zK&W] %s is currently busy with %s boo!", ch->name,
				 ch->sex == SEX_MALE ? "his" : ch->sex == SEX_FEMALE ? "her"
																	 : "its");
		ch->pcdata->afkmess = str_dup(buf);
		return;
	}
...
CMDF(do_slist)
{
	...
	if (argument[0] == '\0')
	{
		...
		send_to_char("&RType&r: &pcombat&P,&p piloting&P,&p engineering&P,&p smuggling&P,&p bounty&P,\r\n &p diplomacy&P,&p leadership&P,&p espionage&P,&p all.\r\n",
					 ch);
		return;
	}
	...
	for (ability = -1; ability < MAX_ABILITY; ability++)
	{
		...
		for (i = lowlev; i <= hilev; i++)
		{
			for (sn = 0; sn < top_sn; sn++)
			{
				...
				{
					pager_printf(ch, "&C[%s%-18.18s &B%3d&C] ", !IS_IMMORTAL(ch) ? "&c" : !get_help(ch, skill_table[sn]->name) ? "&B"
																															   : "&c",
								 capitalize(skill_table[sn]->name), i);
					++cnt;
					if (++col == 3)
					{
						pager_printf(ch, "\r\n");
...
CMDF(do_whois)
{
	...
	*/
	...
	ch_printf(ch, "&CRace&c:&z %-9s          &CSex&c:&z %-6s\r\n",
			  pc_race[victim->race],
			  victim->sex == SEX_MALE ? "&BMale" : victim->sex == SEX_FEMALE ? "&PFemale"
																			 : "&WNonbinary");
	...
	*/
	...
	if (IS_IMMORTAL(ch))
	{
		...
		if (xIS_SET(victim->act, PLR_SILENCE) || xIS_SET(victim->act, PLR_NO_EMOTE) || xIS_SET(victim->act, PLR_NO_TELL))
		{
